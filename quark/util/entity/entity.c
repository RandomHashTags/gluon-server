...
void entity_tick(struct Entity *entity) {
    ...
    if (entity->is_in_block_passthroughable) {
        ...
        chunk_get_loaded_block_at_xyz(chunk, block_x, block_y, block_z, block_at_location);
    }
    
    if (entity->is_affected_by_gravity) {
        if (entity->fall_distance_total == 0) { // TODO: remove this | for testing only
            entity->is_on_ground = 0;
            entity->fall_distance = 1;
            entity->fall_distance_total = 10;
        }
        if (entity->is_on_ground) {
            // TODO: check if falling - if so, calculate and set distance
            if (!calculated_block_at_location) {
                calculated_block_at_location = 1;
                const long block_x = (long) location->x, block_y = (long) location_y, block_z = (long) location->z;
                chunk_get_loaded_block_at_xyz(chunk, block_x, block_y, block_z, block_at_location);
            }
            if (!block_at_location || block_at_location->material->configuration->block->can_passthrough) {
            }
        } else {
            const float current_fall_distance = entity->fall_distance;
            const float new_fall_distance = current_fall_distance - GRAVITY_PER_TICK;
            printf("entity_tick; entity with uuid %d isn't on ground. current_fall_distance=%f, new_fall_distance=%f\n", entity->uuid, current_fall_distance, new_fall_distance);
            if (new_fall_distance <= 0.00) {
                ...
                if (calculate_fall_damage) {
                    ...
                    if (!calculated_block_at_location) {
                        calculated_block_at_location = 1;
                        chunk_get_loaded_block_at_xyz(chunk, location->x, location_y, location->z, block_at_location);
                    }
