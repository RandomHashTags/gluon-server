...
struct GluonServer *SERVER;

#define GLUON_SERVER_THREAD_COUNT 3
#define GLUON_SERVER_THREAD_ID_HANDLE_PLAYER_CONNECTIONS 0
#define GLUON_SERVER_THREAD_ID_SERVER_LOOP 1
#define GLUON_SERVER_THREAD_ID_UPDATE_PLAYER_PINGS 2
pthread_t *THREADS;

void server_create(void) {
    struct GluonServer *server = malloc(sizeof(struct GluonServer));
    if (!server) {
        printf("failed to allocate memory for a GluonServer\n");
        return;
    }
    server->is_sleeping = 1;
    
    unsigned short plugins_count = 0;
    struct GluonPlugin **plugins = malloc(plugins_count * sizeof(struct GluonPlugin *));
    if (!plugins) {
        free(server);
        printf("failed to allocate memory for a GluonServer plugins\n");
        return;
    }
    printf("server registered %d GluonPlugins\n", plugins_count);
    
    struct World **worlds = malloc(4 * sizeof(struct World *));
    if (!worlds) {
        free(server);
        free(plugins);
        printf("failed to allocate memory for a GluonServer worlds\n");
        return;
    }
    ...
    if (!enchantment_types) {
        ...
        free(inventory_types);
        free(materials);
        printf("failed to allocate memory for %d GluonServer enchantment_types\n", enchantment_types_count);
        return;
    }
    ...
    if (!biomes) {
        ...
        printf("failed to allocate memory for %d GluonServer biomes\n", biomes_count);
        return;
    }
...
void server_deallocate(void) {
    ...
    struct GluonPlugin **plugins = SERVER->plugins;
    for (unsigned short i = 0; i < plugin_count; i++) {
        struct GluonPlugin *plugin = plugins[i];
...
void server_start() {
    pthread_t *threads = malloc(GLUON_SERVER_THREAD_COUNT * sizeof(pthread_t));
    if (!threads) {
        printf("failed to allocate memory for the %d threads\n", GLUON_SERVER_THREAD_COUNT);
        return;
    }
    THREADS = threads;
    pthread_create(&THREADS[GLUON_SERVER_THREAD_ID_HANDLE_PLAYER_CONNECTIONS], NULL, begin_connecting_players, (void *) GLUON_SERVER_THREAD_ID_HANDLE_PLAYER_CONNECTIONS);
    pthread_join(THREADS[GLUON_SERVER_THREAD_ID_HANDLE_PLAYER_CONNECTIONS], NULL);
}
void server_stop(void) {
    printf("stopping server...\n");
    server_deallocate();
    pthread_kill(THREADS[GLUON_SERVER_THREAD_ID_HANDLE_PLAYER_CONNECTIONS], GLUON_SERVER_THREAD_ID_HANDLE_PLAYER_CONNECTIONS);
    pthread_kill(THREADS[GLUON_SERVER_THREAD_ID_SERVER_LOOP], GLUON_SERVER_THREAD_ID_SERVER_LOOP);
    pthread_kill(THREADS[GLUON_SERVER_THREAD_ID_UPDATE_PLAYER_PINGS], GLUON_SERVER_THREAD_ID_UPDATE_PLAYER_PINGS);
    server_destroy();
}
void server_set_sleeping(_Bool value) {
    ...
    if (value != previous_value) {
        if (!value) {
            pthread_create(&THREADS[GLUON_SERVER_THREAD_ID_SERVER_LOOP], NULL, begin_ticking_server, (void *) GLUON_SERVER_THREAD_ID_SERVER_LOOP);
            pthread_create(&THREADS[GLUON_SERVER_THREAD_ID_UPDATE_PLAYER_PINGS], NULL, begin_updating_player_pings, (void *) GLUON_SERVER_THREAD_ID_UPDATE_PLAYER_PINGS);
        } else {
            server_stop();
        }
    }
...
void server_change_tick_rate(const unsigned char ticks_per_second) {
    ...
    printf("server will change tick rate for worlds...\n");
    const unsigned short world_count = SERVER->world_count;
    struct World **worlds = SERVER->worlds;
    for (unsigned short i = 0; i < world_count; i++) {
        struct World *world = worlds[i];
...
struct GluonPlugin *server_get_plugin(const char *name, const char *bundle_id) {
...
struct World *server_world_create(const long seed, const char *world_name, struct Difficulty *difficulty) {
    ...
    if (world_count + 1 > world_count_maximum) {
        ...
        if (!worlds) {
            printf("failed to reallocate memory to expand GluonServer worlds!\n");
            return NULL;
        }
    ...
    worlds[world_count] = world;
    printf("server_world_create; created world \"%s\" at address %p (server is tracking world at address %p)\n", world->name, world, worlds[world_count]);
...
struct Player *server_parse_player(unsigned int uuid) {
    ...
    // TODO: parse values from local json, or create default
...
void server_player_joined(struct PlayerConnection *connection) {
    ...
    world_player_joined((struct World *) world, connection);
